...
public class BlockPressurePlateDC extends BlockBasePressurePlate {
	...
	public BlockPressurePlateDC(Material materialIn, boolean b, String name) {
		super(materialIn);
		this.setDefaultState(this.blockState.getBaseState().withProperty(DCStateBase.POWERED, Boolean.valueOf(false)));
		this.isPlayerOnly = b;
	...
	protected int getRedstoneStrength(IBlockState state) {
		return state.getValue(DCStateBase.POWERED).booleanValue() ? 15 : 0;
	}

	@Override
	protected IBlockState setRedstoneStrength(IBlockState state, int strength) {
		return state.withProperty(DCStateBase.POWERED, Boolean.valueOf(strength > 0));
	}

	@Override
	protected void playClickOnSound(World world, BlockPos color) {
		world.playSound((EntityPlayer) null, color, SoundEvents.BLOCK_STONE_PRESSPLATE_CLICK_ON, SoundCategory.BLOCKS,
				0.3F, 0.6F);
	}

	@Override
	protected void playClickOffSound(World world, BlockPos pos) {
		world.playSound((EntityPlayer) null, pos, SoundEvents.BLOCK_STONE_PRESSPLATE_CLICK_OFF, SoundCategory.BLOCKS,
				0.3F, 0.5F);
	}
	...
	public IBlockState getStateFromMeta(int meta) {
		return this.getDefaultState().withProperty(DCStateBase.POWERED, Boolean.valueOf(meta == 1));
	}

	@Override
	public int getMetaFromState(IBlockState state) {
		return state.getValue(DCStateBase.POWERED).booleanValue() ? 1 : 0;
	}
	...
	public void addInformation(ItemStack stack, @Nullable World player, List<String> tooltip, ITooltipFlag advanced) {
		...
		tooltip.add(I18n.format("dcutil.tip.pressure_plate"));
