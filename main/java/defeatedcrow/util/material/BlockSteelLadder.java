...
public class BlockSteelLadder extends DCBlockBase {
	...
	public BlockSteelLadder(String s) {
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(DCStateBase.FACING,
				EnumFacing.SOUTH).withProperty(CLAMP, false).withProperty(UPPER, false));
	}
		...
		if (!player.world.isRemote && player != null) {
			...
			if (!held.isEmpty() && held.getItem() == Item.getItemFromBlock(this)) {
				...
				if (target != null) {
					...
					IBlockState set = this.getDefaultState().withProperty(DCStateBase.FACING, face);
	...
	public IBlockState withRotation(IBlockState state, Rotation rot) {
		...
		switch (rot) {
		case CLOCKWISE_180:
			return state.withProperty(DCStateBase.FACING, face.getOpposite());
		case COUNTERCLOCKWISE_90:
			return state.withProperty(DCStateBase.FACING, face.rotateY());
		case CLOCKWISE_90:
			return state.withProperty(DCStateBase.FACING, face.rotateYCCW());
		default:
			return state;
		}
	}
	...
	public IBlockState getPlaceState(World world, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ,
		...
		if (placer != null) {
			...
			state = state.withProperty(DCStateBase.FACING, face);
		} else {
			if (facing == EnumFacing.DOWN || facing == EnumFacing.UP) {
				facing = EnumFacing.NORTH;
			}
			state = state.withProperty(DCStateBase.FACING, facing.getOpposite());
		}
		return state;
	}
	...
	public IBlockState getStateFromMeta(int meta) {
		int i = meta & 3;
		IBlockState state = this.getDefaultState().withProperty(DCStateBase.FACING, EnumFacing.getFront(5 - i));
		return state;
	}
	...
	public int getMetaFromState(IBlockState state) {
		int i = 5 - state.getValue(DCStateBase.FACING).getIndex();
		return i;
	}
