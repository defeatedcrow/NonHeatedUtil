	...
	protected void onServerUpdate() {
		...
		if (!world.isRemote) {
			...
			if (!DCStateBase.getBool(state, DCStateBase.POWERED))
				...
				if (fromTE == null || !fromTE.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY,
						face.getOpposite())) {
					fromTE = world.getTileEntity(getPos().offset(face, 2));
				}
				TileEntity toTE = world.getTileEntity(getPos().down());
				if (fromTE != null && toTE != null && fromTE.hasCapability(
						CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, face.getOpposite()) && toTE.hasCapability(
